...
export class Length extends Expresion{
    ...
    public execute(env: Environment): Retorno {
        ...
        if (nodoExp.type == Tipo.ERROR) {
            ...
            const error = new Excepcion("Error semántico", `No es posible aplicar el método Length() a un valor devuelto de tipo NULL`, this.line, this.column);
            consola.set_Error(error);
            retorno = {value: null, type: Tipo.ERROR}
        }else{
            if(nodoExp.type == Tipo.STRING){
                retorno = { value: (nodoExp.value.toString()).length, type: Tipo.INT}
            }else{
                //no tiene que hacer la conversión y solo dar un error semántico
                const error = new Excepcion("Error semántico", `No es posible aplicar el método Length() solo se puede aplicar a cadenas`, this.line, this.column);
                consola.set_Error(error);
                retorno = {value: null, type: Tipo.ERROR}
            }
        }
