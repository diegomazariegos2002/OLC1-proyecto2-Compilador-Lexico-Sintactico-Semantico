...
export class Multiplicacion extends Expresion{
    ...
    public execute(env: Environment): Retorno {
        ...
        if (nodoIzq.type == Tipo.ERROR || nodoDer.type == Tipo.ERROR) {
            ...
            const error = new Excepcion("Error semántico", `NULL multiplicado por NULL no es una multiplicación válida.`, this.line, this.column);
            ...
            if(nodoIzq.type == nodoDer.type){ //si ambas expresiones son del mismo tipo.
                if (nodoIzq.type == Tipo.INT) {
                    ...
                    //Error
                    const error = new Excepcion("Error semántico", `boolean\\caracter\\cadena\\NULL multiplicado por boolean\\caracter\\cadena\\NULL no es una multiplicación válida.`, this.line, this.column);
                    consola.set_Error(error);
                    retorno = {value: null, type: Tipo.ERROR}
                }
                ...
                if (nodoDer.type == Tipo.DOUBLE) {
                    ...
                    //ERROR
                    const error = new Excepcion("Error semántico", `int multiplicado por boolean\\string\\NULL no es una multiplicación válida.`, this.line, this.column);
                    consola.set_Error(error);
                    retorno = {value: null, type: Tipo.ERROR}
                }
                ...
                if (nodoDer.type == Tipo.INT) { // Si el otro es tipo int
                    ...
                    //ERROR
                    const error = new Excepcion("Error semántico", `double multiplicado por boolean\\string\\NULL no es una multiplicación válida.`, this.line, this.column);
                    consola.set_Error(error);
                    retorno = {value: null, type: Tipo.ERROR}
                }
                ...
                if (nodoDer.type == Tipo.INT) {
                    ...
                    //ERROR
                    const error = new Excepcion("Error semántico", `char multiplicado por boolean\\string\\NULL no es una multiplicación válida.`, this.line, this.column);
                    consola.set_Error(error);
                    retorno = {value: null, type: Tipo.ERROR}
                }
            }else{ //Cualquier otro tipo de error
                //ERROR
                const error = new Excepcion("Error semántico", `la multiplicación de un boolean\\string\\NULL no es una multiplicación válida.`, this.line, this.column);
                consola.set_Error(error);
                retorno = {value: null, type: Tipo.ERROR}
            }
        }
